...
export default function LivePage() {
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden text-white" onClick={() => setShowPopup(true)}>
      ...
      <div className="absolute bottom-40 left-4 z-20" onClick={(e) => e.stopPropagation()}>
      ...
      {/* CHAT INPUT */}
      <div className="absolute bottom-24 left-4 right-4 flex items-center gap-3 z-20" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-md px-4 py-3 flex items-center gap-3 z-30 border-t border-white/10" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="bg-pink-400 text-black text-xs px-4 py-2 rounded-full font-bold">Beli</button>
      </div>
